...
\end{equation}
{with primal-dual splitting algorithms and its coordinate updates, where $f$ is differentiable %, $g$ and $h$ are possibly nondifferentiable, 
...
In many applications, although $h$ is  proximable, $h\circ A$  is generally non-proximable and non-differentiable. To avoid using the slow subgradient method,  we can consider the  primal-dual splitting approaches to separate $h$ and $A$ so that $\prox_{h}$ can be applied. 
...
\begin{remark}
Similar primal-dual algorithms can be used to solve  problems  that are more other than $\eqref{pdproblem}$ such as saddle point problems~\cite{lebedev1967duality,mclinden1974extension,briceno2013monotone} and variational inequalities~\cite{tseng1991applications}. Our coordinate update algorithms below apply to these problems as well.
\end{remark}
...
\text{subject to } A_1x_1+A_2x_2+\cdots+A_mx_m=b,
\end{array}\label{emp}
\end{equation}
...
%\right.
%\end{equation}
Applying the iteration $\eqref{vucondat}$ to the problem $\eqref{emp}$ and eliminating $s^{k+1}$ from the second row yields the Jacobi-style update (denoted as $\cT_\textnormal{emp}$):
\begin{equation}
\left\{
\begin{array}{l}
s^{k+1}=s^k+\gamma (Ax^k-b)\\
...
\cI\\\prox_{\eta g}
\end{bmatrix},\quad \cT_2\begin{bmatrix}
s\\x
\end{bmatrix}=\begin{bmatrix}
s+\gamma (Ax-b)\\
...
{In the coordinate update scheme proposed in \S\ref{sec:pdcu}, updating $x_i$ involves computing $s^{k+1}$ but the result is discard (see the proof of Proposition \ref{prop1}, item 1). This is because of  $x_i$'s and $s_j$'s are coupled through the matrix $A$.
...
Typical choices of $\rho_{i,j}$ include: (1) one of the $\rho_{i,j}$'s is 1 for each $j$, others all equal to 0. This can be viewed as assigning $s_j$ fully to the block containing $x_i$.
%\begin{equation}
%\left\{
%\begin{array}{l}
%\text{when }x_i\text{ is chosen to be updated, compute}\\
...
\end{equation}
has the same solution as $\eqref{pdproblem}$. The they solve $\eqref{fbpdproblem}$ by the scheme $\eqref{pdoverlap}$. Because they have $m_j=1$, every dual variable coordinate is only associated with one primal variable coordinate. 
...
%
%%\subsection{the duplication trick}
%%In their paper \cite{?}, Fercoq and Bianchi introduced a duplication trick, using which they change problem $\eqref{pdproblem}$(with $l=0$) to an equivalent one, which they can solve using algorithm $\eqref{vucondat}$ with $m_j=1$ for every $j$. We illustrate their idea briefly here.
%%We define $\cG_j=\otimes_{r\in\II(j)}\GG_j,\forall j=1,2,\ldots,p$ with $m_j$ copies of $\GG_j$, and $\cG=
%%\cG_1\times \cG_2\times\cdots\cG_p$. We write elements of $\cG_j$ as $\vs_j=(\vs_j(i_1),\ldots,\vs_j(i_{m_j}))$ with $i_r\in\II(j)$, and elements of $\cG$ as $\vs=(\vs_1,\ldots,\vs_p)$. Define $D(m)=\text{diag}(m_1,\ldots,m_p)$, Fercoq and Bianchi proved there exists a (column averaging) linear operator $S:\cG\to\GG$ and a linear operator $K:\HH\to\cG$, such that $A=D(m)SK$ and $K$ couples every small block $\vs_j(r)$ of $\vs\in\cG$ with only one block of $x$. They defined $\bar{h}=h\circ(D(m)S)$, then problem $\eqref{pdproblem}$ becomes 
%%\begin{equation}
%%\underset{x\in\mathbb{H}}{\text{minimize }}  f(x)+g(x)+\bar{h}(Kx) 
%%\end{equation} 
%%We can develop a block coordinate update algorithm using the second or the third scheme listed above and get the algorithm in \cite{?}:
%%\begin{equation}
%%\left\{
%%\begin{array}{l}
%%\text{choose }x^0\in\HH,\vs^0\in\cG\\
%%\text{For all }i=1,2,\ldots,m \text{, define }w_i^0=\sum_{j\in\JJ(i)}A^\top_{j,i}\vs_j^{0}(i)\\
%%\text{For all }j=1,2,\ldots,p \text{, define }t_j^0=\frac{1}{m_j}\sum_{i\in\II(j)}\vs_j^{0}(i)\\
%%\text{when }x_i\text{ is chosen to be updated, compute}\\
%%\tilde{s_j}^{k+1}=\prox_{\gamma h_j^*} (t_j^k+\gamma Ax^k),\forall j\in\JJ(i)\\
%%\tilde{x_i}^{k+1}=\prox_{\eta g_i}(x_i^k-\eta(\nabla_i f(x^k)+2\sum_{j\in\JJ(i)}A_{j,i}^\top\tilde{s}_j^{k+1}-w_i^k))\\
%%\text{update }x_i^{k+1}=x_i^k+\eta_k(\tilde{x}_i^{k+1}-x_i^k) \\
%%\text{update }\vs_j^{k+1}(i)=\vs_j^k(i)+\eta_k(\tilde{s}_j^{k+1}-\vs_j^k(i)),\forall j\in\JJ(i)\\
%%\text{update }w_i^{k+1}=w_i^k+\sum_{j\in\II(j)}A^\top_{j,i}(\vs_j^{k+1}(i)-\vs_j^{k}(i))\\
%%\text{update }t_j^{k+1}=t_j^k+\frac{1}{m_j}(\vs_j^{k+1}(i)-\vs_j^{k}(i)),\forall j\in\JJ(i)
%%\end{array}
%%\right.
%%\end{equation}
%%Next we consider a special case of $\eqref{pdproblem}$ which also has a lot of applications. We will see in that case the primal variable $x$ and the dual variable $s$ can be totally decoupled.

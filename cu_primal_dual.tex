...
\rev{
In this section, we propose two async-parallel primal dual coordinate update algorithms based on~\cite{Peng_2015_AROCK} and state their convergence results. 
...
The two algorithms are based on Sections~\ref{sec:pdcu} and~\ref{sec:overlap}, respectively.
...
The async-parallel algorithm using the overlapping-block coordinate update~\eqref{pdoverlap} is in Algorithm~\ref{alg:asyn_overlap}.
...
Let $Z^*$ be the set of optimal solutions of problem~\eqref{pdproblem} and $(z^k)_{k\geq0}\subset \FF$ be the sequence generated by Algorithm \ref{alg:asyn_core} or Algorithm \ref{alg:asyn_overlap} under the following conditions:
\begin{enumerate}[(i)]
...
\end{thm} 
The explicit formulas for $\eta_{\min}$ and $\eta_{\max}$, the proof of Theorem~\ref{thm:async-convergence}, and additional remarks are included in Appendix \ref{pf:pdasync}.
}

...
\end{thm}
The explicit formulas for $\eta_{\min}$ and $\eta_{\max}$, the proof of Theorem~\ref{thm:async-convergence}, and additional remarks are included in Appendix \ref{pf:pdasync}. The algorithms can be applied to solve  problems formulated as~\eqref{pdproblem}. A variety of examples are provided in~\S\ref{sec:ML} and~\S\ref{sec:Imaging}.

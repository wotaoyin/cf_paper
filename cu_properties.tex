...
We can implement the coordinate update in Example \ref{ex:lsq1} in a different manner by maintain the result $\cT x^k$ throughout, which works when $m=\Theta(n)$ or $p\gg m$. The full update is unchanged. 
...
For the case $p\ll m$, i.e., the system $Ax=b$ is highly undetermined, we should avoid pre-computing $A^\top A$ because multiplying $A$ and then $A^\top$ is cheaper. Therefore, we change the implementations of both the full and coordinate updates in Example \ref{ex:lsq1}. In particular, the full update
...
Let $E$ be a class of index sets and every $e\in E$ be a small subset of $\{1, ..., m\}$, $|e|\ll m$. Let $x_e:=(x_i)_{i\in e}$, and  

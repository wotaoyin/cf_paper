...
\item Let $v\in\RR^{n}$ and $A=[a_1~A_2]\in\RR^{m\times n}$, where $a_1\in\RR^m,A_2\in\RR^{m\times (n-1)}$. Given $\rho_1^v,\rho_2^v,\xi_1^v,\xi_2^v$, we have $$A(2\cdot\prj_{Q}(v)-v)=((2\xi_1 ^v-1)v_1)\cdot a_1 + (2\xi_2^v-1)\cdot A_2 (v_2,\ldots,v_n)^T.$$
\end{enumerate}
\end{proposition}

...
\rev{
Our algorithm~\ref{alg:asyn_core} and~\ref{alg:asyn_overlap} differ from that in~\cite{Peng_2015_AROCK} in the following aspects:
\begin{enumerate}
\item the operator $\TVC$ is nonexpansive in a norm induced by a symmetric positive definite matrix $M$ (see Appendix~\ref{sec:vc-op}), instead of the Euclidean norm;
\item the coordinate updates we make are no longer orthogonal to each other in the norm induced by $M$;
\item the block coordinates we update at each iteration may overlap with each other.
\end{enumerate}
Because of these differences, we make two major modifications to the proof in \cite[Section 3]{Peng_2015_AROCK}: (i) adjust parameters in \cite[Lemma 2]{Peng_2015_AROCK} and modify its proof to accommodate for the new norm; (2) modify the inner product and induced norm used in \cite[Theorem 2]{Peng_2015_AROCK} and adjust the constants in \cite[Theorem 2\& 3]{Peng_2015_AROCK}.

In order to state these modifications, we introduce some notation first. First since $\hat{z}^k_i$ can be related to $z_i^k$ through the interim changes applied to $z_i$, we let $J_i(k)\subset \{k-1,...,k-\tau\}$ be the index set of these interim changes. We have
\begin{equation}
...
Define $S=I-\TVC$, by Lemma~\ref{lemma:a-avg} below, $\TVC$ is nonexpansive in the norm induced by $M$ if and
only if $S$ is ${1/2}$-cocoercive in the same norm.
\begin{lemma}\label{lemma:a-avg}
(Using any norm), operator $T:\FF\to\FF$ is nonexpansive if and only if $S = I - T$ is {${1}/{2}$-cocoercive}, i.e.,
\begin{equation}\label{eqn:alpha_avg}
\langle z-\tilde{z},Sz- S\tilde{z}\rangle_M \ge \frac{1}{2}\|Sz- S\tilde{z}\|_M^2,\quad
{\forall~z,\tilde{z}\in\FF}.
\end{equation}
\end{lemma}
The proof is the same as that of \cite[Proposition 4.33]{bauschke2011convex}. It is shown in \cite{davis2014convergence} that with proper choice of $\eta$ and $\gamma$, $\TVC$ is nonexpansive in the norm induced by $M$. Let $\lambda_{\max},\lambda_{\min}$ be the
maximal and minimal eigenvalues of the matrix $M$, and $\kappa=\frac{\lambda_{\max}}{\lambda_{\min}}$ be the condition number.
...
\end{align}
and let $|J(k)|$ be the number of elements in $J(k)$, $q_{\min}=\min_iq_i>0$. 
...
\end{thm}
The proof directly follows from \cite[Section 3]{Peng_2015_AROCK}. Here we only present the key modifications. Interested readers are referred to the orginal paper for the complete procedure.
...
Let $(z^k)_{k\geq 0}$ be the sequence generated by ARock. Then for any $\vz^*\in\vZ^*$, it holds that %the following fundamental inequality% for both consistent read and inconsistent read

...
y^{k+1}\\
z^{k+1}
\end{pmatrix}&=\argmin_{(y,z)^T}g(y)+h(z)+\frac{\eta}{2}\|Ax^{k+1}+By+Cz+\frac{s^k}{\eta}-b\|^2,\\
...
Next, we discuss how $\cT_1\circ \cT_2$ inherits the CF properties from $\cT_1$ and $\cT_2$, and the results are summarized in Table \ref{table:comp-op}. For simplicity, we only use matrix-vector multiplication as examples in this subsection; more examples will be given later.%where at least one operator in the composition is non-separable. 
\begin{itemize}
...
x^{k+1} = x^k - \eta_k \left(\lambda  x^k + \sum_{i = 1}^p \frac{-b_i}{1 + \exp{(b_i \cdot a_i^T x^k)}} a_i\right),
\end{equation}

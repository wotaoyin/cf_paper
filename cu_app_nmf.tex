...
Partitioning the variables into $2p$ block coordinates: $(w_1, \ldots, w_p, h_1,\ldots, h_p)$, we can apply the coordinate update based on the projected-gradient method: 
\begin{equation}\label{pcg-nmf}
{\left\{
\begin{array}{l}
\text{if $w_{i_k}$  is chosen for some $i_k \in [p]$, then compute}\\
\qquad w_{i_k}^{k+1}=\max\big(0, w_{i_k}^k-\eta_k\nabla_{w_{i_k}} F(W^k, H^k)\big) \\
\text{if $h_{i_k-p}$  is chosen for some $i_k \in \{p+1, ..., 2p\}$, then compute}\\
\qquad h_{i_k-p}^{k+1}=\max\big(0, h_{i_k-p}^k-\eta_k \nabla_{h_{i_k-p}} F(W^k, H^k) \big) 
\end{array}
\right.
}\end{equation}
...
\frac{1}{\|h_{i_k}^k\|_2^2},&\text{ if }1\le i_k\le p,\\[0.1cm]
\frac{1}{\|w_{i_k-p}^k\|_2^2},&\text{ if }p+1\le i_k\le 2p.
\end{array}\right.$$
